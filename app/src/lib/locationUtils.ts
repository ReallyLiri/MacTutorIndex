...
export function getAllSelectedPaths(selectedPaths: string[]): string[] {
...
export function getNodeSelectionState(
  ...
  if (node.children.length === 0) {
    return selectedSet.has(node.fullPath) ? "checked" : "unchecked";
  }

  const isAllSelected = (n: LocationNode): boolean => {
    if (selectedSet.has(n.fullPath)) return true;
    return n.children.length > 0 && n.children.every(isAllSelected);
  };

  const isSomeSelected = (n: LocationNode): boolean => {
    if (selectedSet.has(n.fullPath)) return true;
    return n.children.some(isAllSelected);
  };

  if (node.children.every(isAllSelected)) {
    return "checked";
  }
  if (node.children.some(isSomeSelected)) {
